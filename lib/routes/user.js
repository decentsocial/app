...
router.get('/timeline', async function (req, res, next) {
  ...
  const following = settings.following ? settings.following : await getFollowing(twitterHandle)
  ...
  if (!settings.following) {
    await SettingsRepo.update({ _id: settings._id }, { $set: { following } })
      .then(() => console.log('updated user following', user, following))
      .catch(err => {
        console.error('failed to update user following', user, following)
        console.error(err)
      })
  }
